        ...
        static void GuardarDatos()
        {
            using (StreamWriter sw = new StreamWriter("empleados.txt"))
            {
                foreach (var empleado in empleados)
                {
                    ...
                    sw.WriteLine(empleado.Nombre); // Escribir el nombre del empleado
                    sw.WriteLine(empleado.Salario); // Escribir el salario del empleado
                    if (empleado is EmpleadoPorHoras)
                    {
                        sw.WriteLine(((EmpleadoPorHoras)empleado).HorasTrabajadas); // Escribir las horas trabajadas si es empleado por horas
                        sw.WriteLine(((EmpleadoPorHoras)empleado).SalarioPorHora); // Escribir el salario por hora si es empleado por horas
                    }
                }
            }
